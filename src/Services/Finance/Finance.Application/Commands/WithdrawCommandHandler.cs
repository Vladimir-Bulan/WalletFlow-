...
public class WithdrawCommandHandler : IRequestHandler<WithdrawCommand, TransactionDto> {
    ...
    public WithdrawCommandHandler(IAccountRepository repository, IEventStore eventStore) { _repository = repository; _eventStore = eventStore; }
    public async Task<TransactionDto> Handle(WithdrawCommand request, CancellationToken ct) {
        var account = await _repository.GetByIdAsync(request.AccountId, ct) ?? throw new AccountNotFoundException(request.AccountId);
        var transaction = account.Withdraw(Money.Of(request.Amount, request.Currency), request.Description);
